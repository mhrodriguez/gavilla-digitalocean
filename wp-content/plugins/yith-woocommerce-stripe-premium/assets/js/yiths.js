...
(function ( $ ) {
    ...
    function stripeFormHandler( event ) {
        var $form = $( 'form.checkout, form#order_review, form.add-card' );

        if ( $form.hasClass('add-card') || $( 'input#payment_method_yith-stripe' ).is( ':checked' ) && ( $( 'input#yith-stripe-pay-card-new' ).is( ':checked' ) || $( 'input[name="yith-stripe-pay-card"]').val() == 'new' ) ) {
            ...
            if ( 0 === $( 'input.stripe-token' ).size() ) {
                ...
                if ( error ) {
                    ...
                    $('fieldset#yith-stripe-cc-form input, fieldset#yith-stripe-cc-form select').one( 'keydown', function() {
    ...
    $(document).ready(function(){
        ...
        var change_card = function(){
            ...
            if ( $cards.length ) {
                $cards.siblings( 'fieldset#yith-stripe-cc-form').hide();

                $( 'body' ).bind( 'updated_checkout', function() {
                    $( '#payment').find( 'div.cards').siblings( 'fieldset#yith-stripe-cc-form').hide();
                });

                $('form.checkout, form#order_review').on( 'change', '#payment input[name="yith-stripe-pay-card"]', function(){console.log('change');
                    var input = $(this),
                        $cards = $( '#payment').find( 'div.cards');

                    // change selected
                    $cards.find('div.card').removeClass('selected');
                    $cards.find('input[name="yith-stripe-pay-card"]:checked').closest('div.card').addClass('selected');

                    if ( input.val() == 'new' ) {
                        $cards.siblings( 'fieldset#yith-stripe-cc-form').show();
                    } else {
                        $cards.siblings( 'fieldset#yith-stripe-cc-form').hide();
                    }
                });
        ...
        /* Pay Page Form */
        $( 'form#order_review, form.add-card' ).on( 'submit', function (e) {
            if ( mode == 'bitcoin' ) {
                return stripeBitcoinFormHandler(e);
            } else {
                return stripeFormHandler(e);
        ...
        /* Both Forms */
        $( 'form.checkout, form#order_review, form.add-card' ).on( 'change', '#yith-stripe-cc-form input', function() {
                $( '.stripe-token' ).remove();
            })
    ...
    var cvv_lightbox = function(){
        ...
        $('.woocommerce #payment ul.payment_methods li, form.add-card').find( 'a.cvv2-help' ).prettyPhoto({
            hook: 'data-rel',
            social_tools: false,
