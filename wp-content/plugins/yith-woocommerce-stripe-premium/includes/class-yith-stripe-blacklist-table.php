    ...
    class YITH_Stripe_Blacklist_Table extends WP_List_Table {
        ...
        public function get_columns() {
            $columns = array(
                ...
                'ban_status' => '<span class="status_head tips" data-tip="' . esc_attr__( 'Status', 'yith-stripe' ) . '">' . esc_attr__( 'Status', 'yith-stripe' ) . '</span>',
                'user' => __( 'User', 'yith-stripe' ),
                'ip' => __( 'IP Address', 'yith-stripe' ),
	            'order' => __( 'Order', 'yith-stripe' ),
	            'date' => __( 'Date', 'yith-stripe' ),
	            'user_actions' => __( 'Actions', 'yth-stripe' )
            );
	    ...
	    public function get_bulk_actions() {
		    $actions = array(
			    'ban' => __( 'Ban', 'yith-stripe' ),
			    'unban' => __( 'Unban', 'yith-stripe' )
		    );
	    ...
	    protected function get_views() {
		    ...
		    $views = array(
			    'all' => __( 'All', 'yith-stripe' ),
			    'banned' => __( 'Banned', 'yith-stripe' ),
			    'unbanned' => __( 'Unbanned', 'yith-stripe' ),
		    );
		    ...
		    foreach ( $views as $id => $view ) {
			    ...
			    $filter = "0', '1";
			    if ( 'banned' == $id ) $filter = '0';
			    elseif ( 'unbanned' == $id ) $filter = '1';
        ...
        public function column_default( $rec, $column_name ) {
            switch ( $column_name ) {
                ...
                case 'ban_status':
                    $display = $rec->unbanned == 1 ? __( 'Unbanned', 'yith-stripe' ) : __( 'Banned', 'yith-stripe' );
	            ...
	            case 'user_actions':
					$unban_url = wp_nonce_url( add_query_arg( array( 'page' => $_GET['page'], 'tab' => $_GET['tab'], 'action' => 'unban', 'id' => $rec->ID ), admin_url( 'admin.php' ) ), 'stripe_blacklist_action' );
					$ban_url = wp_nonce_url( add_query_arg( array( 'page' => $_GET['page'], 'tab' => $_GET['tab'], 'action' => 'ban', 'id' => $rec->ID ), admin_url( 'admin.php' ) ), 'stripe_blacklist_action' );

		            if ( $rec->unbanned == 0 ) {
			            printf( '<a class="button tips complete" href="%1$s" data-tip="%2$s">%2$s</a>', $unban_url, __( 'Unban', 'yith-stripe' ) );
		            }

		            elseif ( $rec->unbanned == 1 ) {
			            printf( '<a class="button tips" href="%1$s" data-tip="%2$s">%2$s</a>', $ban_url, __( 'Ban', 'yith-stripe' ) );
		            }
