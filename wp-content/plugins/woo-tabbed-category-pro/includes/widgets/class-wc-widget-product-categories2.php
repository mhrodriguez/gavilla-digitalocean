...
class WC_Widget_Product_Categories2 extends WC_Widget
{
    ...
    public function __construct()
    {
        ...
        $this->widget_name = __('Woo Tab Product Category Dropdown', 'woocommerce');
        $this->settings = array(
            'title' => array(
                'type' => 'text',
            ...
            'orderby' => array(
                ...
                'type' => 'select',
                'std' => 'qc_theme_one',
                'label' => __('Select a theme', 'woocommerce'),
                'options' => array(
                    'qc_theme_one' => __('Colorized', 'woocommerce'),
                    'qc_theme_bubba' => __('Bubba', 'woocommerce'),
                    'qc_theme_skinny' => __('Skinny', 'woocommerce'),
                    'qc_theme_transit' => __('Transit', 'woocommerce'),
                    'qc_theme_jet' => __('Jet', 'woocommerce'),
                    'qc_theme_nix' => __('Nix', 'woocommerce'),
                )
            ),
            'dropdown' => array(
                'type' => 'checkbox',
    ...
    public function widget($args, $instance)
    {
        ...
        $qc_theme = isset($instance['qc_theme']) ? $instance['qc_theme'] : $this->settings['qc_theme']['std'];
        $e_c = isset($instance['expand_collapse']) ? $instance['expand_collapse'] : $this->settings['expand_collapse']['std'];


        $this->qc_selected_theme = $qc_theme;
...
//        }
        ...
        if ($d) {
            ...
            if ($e_c) {
                ...
                $list_args['heiararchical'] = true;
                $list_args['echo'] = false;
                echo '<div class="wcc_block statics">';
                echo '<ul class="mtree">';
    ...
    public function wa_wcc_mtree_script()
    {
        ...
        wp_register_script('wa_wcc_mtree', plugins_url('/assets/js/mtree.js', __FILE__), array('jquery'), '');
    ...
    public function wa_wcc_load_scripts()
    {


        if ($this->qc_selected_theme == 'qc_theme_one') {
            wp_register_style('wa_wcc_mtree_css_file', plugins_url('/assets/css/mtree.css', __FILE__));
            wp_enqueue_style('wa_wcc_mtree_css_file');

        } else if ($this->qc_selected_theme == 'qc_theme_bubba') {
            wp_register_style('qc_theme_bubba', plugins_url('/assets/css/bubba.css', __FILE__));
            wp_enqueue_style('qc_theme_bubba');

        } else if ($this->qc_selected_theme == 'qc_theme_skinny') {
            wp_register_style('qc_theme_skinny', plugins_url('/assets/css/skinny.css', __FILE__));
            wp_enqueue_style('qc_theme_skinny');

        } else if ($this->qc_selected_theme == 'qc_theme_transit') {
            wp_register_style('qc_theme_transit', plugins_url('/assets/css/transit.css', __FILE__));
            wp_enqueue_style('qc_theme_transit');

        } else if ($this->qc_selected_theme == 'qc_theme_jet') {
            wp_register_style('qc_theme_jet', plugins_url('/assets/css/jet.css', __FILE__));
            wp_enqueue_style('qc_theme_jet');

        } else if ($this->qc_selected_theme == 'qc_theme_nix') {
            wp_register_style('qc_theme_nix', plugins_url('/assets/css/nix.css', __FILE__));
            wp_enqueue_style('qc_theme_nix');
